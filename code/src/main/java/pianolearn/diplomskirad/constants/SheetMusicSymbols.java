package pianolearn.diplomskirad.constants;

public class SheetMusicSymbols {

    public static final String ledger = "\uE022";

    public static final String barLine = "\uE030";

    public static final String trebleClef = "\uE050";
    public static final String bassClef = "\uE062";

    public static final String time1 = "\uE081";
    public static final String time2 = "\uE082";
    public static final String time3 = "\uE083";
    public static final String time4 = "\uE084";
    public static final String time5 = "\uE085";
    public static final String time6 = "\uE086";
    public static final String time7 = "\uE087";
    public static final String time8 = "\uE088";
    public static final String time9 = "\uE089";

    public static final String flat = "\uE260";
    public static final String natural = "\uE261";
    public static final String sharp = "\uE262";

    public static final String dot = "\uE1E7";

    public static final String wholeNote = "\uE1D2";
    public static final String halfNoteUp = "\uE1D3";
    public static final String halfNoteDown = "\uE1D4";
    public static final String quarterNoteUp = "\uE1D5";
    public static final String quarterNoteDown = "\uE1D6";
    public static final String note8Up = "\uE1D7";
    public static final String note8Down = "\uE1D8";
    public static final String note16Up = "\uE1D9";
    public static final String note16Down = "\uE1DA";
    public static final String note32Up = "\uE1DB";
    public static final String note32Down = "\uE1DC";
    public static final String note64Up = "\uE1DD";
    public static final String note64Down = "\uE1DE";

    public static final String wholeRest = "\uE4E3";
    public static final String halfRest = "\uE4E4";
    public static final String quarterRest = "\uE4E5";
    public static final String rest8 = "\uE4E6";
    public static final String rest16 = "\uE4E7";
    public static final String rest32 = "\uE4E8";
    public static final String rest64 = "\uE4E9";
}